...
[Fact]
public async Task ReceiptPlusCredit_ThatFullySettlesInvoice_ChangesInvoiceToCredited()
{
    await using var test =
        await AccountingTestDatabase.CreateAsync();
    ...
    var invoice =
        await test.SalesInvoices.CreateAndPostAsync(
            ...
            new SalesInvoiceRequest(
                ...
                [
                    new SalesInvoiceLineRequest(
                        Description: "Receipt plus credit status test",
                        Quantity: 1m,
                        UnitPrice: 100m,
                        VatTreatment: VatTreatment.Standard,
                        RevenueAccountId: test.Account("4000").Id)
