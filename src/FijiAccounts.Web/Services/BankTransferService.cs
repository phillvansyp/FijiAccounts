...
public sealed record BankTransferRequest(Guid OrganisationId, Guid FromAccountId, Guid ToAccountId, DateOnly Date, string Reference, string? Description, decimal Amount, Guid? BranchId = null, Guid? DivisionId = null);
public sealed class BankTransferService(ApplicationDbContext db, TenantAccessService access, JournalPostingService posting)
{
    public async Task<BankTransfer> PostAsync(string userId, BankTransferRequest request, CancellationToken ct = default)
    {
        ...
        var journal = await posting.PostAsync(userId, new(request.OrganisationId, request.Date, reference, description, [new(request.ToAccountId, description, request.Amount, 0), new(request.FromAccountId, description, 0, request.Amount)], request.BranchId, request.DivisionId), ct);
        var dimension = journal.Lines.First();
        var transfer = new BankTransfer { OrganisationId = request.OrganisationId, BranchId = dimension.BranchId!.Value, DivisionId = dimension.DivisionId!.Value, FromBankAccountId = request.FromAccountId, ToBankAccountId = request.ToAccountId, TransferDate = request.Date, Reference = reference, Description = description, Amount = request.Amount, PostedJournalId = journal.Id, CreatedByUserId = userId }; db.BankTransfers.Add(transfer); db.AuditEvents.Add(new AuditEvent { OrganisationId = request.OrganisationId, UserId = userId, EventType = "BankTransferPosted", EntityType = nameof(BankTransfer), EntityId = transfer.Id.ToString(), JsonData = JsonSerializer.Serialize(new { transfer.Reference, transfer.Amount, transfer.BranchId, transfer.DivisionId, transfer.FromBankAccountId, transfer.ToBankAccountId, JournalId = journal.Id }) }); await db.SaveChangesAsync(ct); await transaction.CommitAsync(ct); return transfer;
    }
}
