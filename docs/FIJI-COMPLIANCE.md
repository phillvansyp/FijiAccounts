...
| Record retention | Seven-year deletion safeguards and export audit events implemented for contact documents, supplier-bill attachments, bank-statement source files and their imported transaction batches. Retained files are written through a provider-neutral, append-only object store with tenant-scoped references and SHA-256 verification on read; legacy rows are backfilled transactionally and idempotently after migration. | Obtain practitioner review and configure an external write-once provider before production accreditation |
