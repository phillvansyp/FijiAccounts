...
| Record retention | Seven-year deletion safeguards and export audit events implemented for contact documents, supplier-bill attachments, bank-statement source files and their imported transaction batches. Retained files are written through a provider-neutral, append-only object store; legacy rows are backfilled transactionally. Daily and on-demand integrity scans verify SHA-256 hashes, lengths, tenant references and coverage, with an audit record, notification and organisation report. | Obtain practitioner review and configure an external write-once provider before production accreditation |
